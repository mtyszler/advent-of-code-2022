...
def test_parse_containers():
    df = parse_initial_locations('input_files/input_day_05a_example.txt')
    assert (df['2'].iloc[1] == 'C')


def test_parse_movement_containers():
    movements = parse_movements_container('input_files/input_day_05b_example.txt')
    assert (movements[1]['move_from'] == 1)
    assert (movements[1]['move_amount'] == 3)


def test_move_one_item():
    container = parse_initial_locations('input_files/input_day_05a_example.txt')
